...
Inception_V3_MODEL_PATH = "model/Inception_V3/"
Inception_V3_MODEL_NAME = "Inception_V3_train_step"
Inception_V3_MODEL_RECORD = "Inception_V3_model_record.json"
Inception_V3_TRAIN_RECORD = "Inception_V3_train_record.png"
Inception_V3_TENSORBOARD_PATH = "tensorboard/Inception_V3/"
...
class Inception_V3:
    def __init__(self, is_training=True):
        ...
        self.val_acc_list = []
        self.val_loss_list = []
        self.model_path = Inception_V3_MODEL_PATH
        self.model_name = Inception_V3_MODEL_NAME
        self.model_record = Inception_V3_MODEL_PATH + Inception_V3_MODEL_RECORD
        self.train_record = Inception_V3_MODEL_PATH + Inception_V3_TRAIN_RECORD
        self.tensorboard = False
        if self.tensorboard:
            self.tensorboard_path = Inception_V3_TENSORBOARD_PATH
            self.feature_map = {}
    ...
    def build_with_slim(self):
        ...
        with slim.arg_scope(inception_v3.inception_v3_arg_scope()):
            self.output, self.end_points = inception_v3.inception_v3(
                self.input_images, self.num_classes, self.is_training, self.keep_prob)
    ...
    def train(self):
        ...
        while self.epoch < self.max_epoch:
            ...
            while self.caltch_256.can_get_train_batch():
                batch_start = datetime.now()
                batch += 1
                self.learning_rate = 0.01 if self.train_step < 5000 else (
                    0.001 if self.train_step < 1000000 else 0.0001)
                self.train_step += 1
